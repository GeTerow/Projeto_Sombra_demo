...
import { DocumentPlusIcon, ArrowDownTrayIcon, PaperAirplaneIcon, PlusIcon, SearchIcon, PencilIcon, TrashIcon, LightbulbIcon } from '../../components/icons';
        ...
        <div className="relative">
            <div className="relative flex flex-col md:flex-row max-w-7xl mx-auto p-4 sm:px-6 lg:px-8 gap-8 min-h-screen">
                <aside className="w-full md:w-1/3 lg:w-1/4">
                    <div className="bg-white/70 dark:bg-slate-800/60 backdrop-blur-md rounded-2xl shadow-xl border border-slate-200/70 dark:border-slate-700/50 p-4 sticky top-6">
                        <div className="flex items-center justify-between mb-3">
                            ...
                            <button
                                disabled
                                title="Desativado no modo de demonstração"
                                className="inline-flex items-center gap-2 px-3 py-2 text-sm font-semibold rounded-lg bg-primary-600 hover:bg-primary-700 text-white shadow-sm disabled:bg-slate-400 dark:disabled:bg-slate-600 disabled:cursor-not-allowed">
                                ...
                                {saleswomen.map(s => {
                                    ...
                                    return (
                                        <li key={s.id} className="group relative">
                                            ...
                                            <div className="absolute top-1/2 -translate-y-1/2 right-3 flex items-center gap-1 opacity-50 cursor-not-allowed">
                                                <button disabled title="Desativado no modo de demonstração" className="p-1.5 rounded-md"><PencilIcon className={`w-4 h-4 ${isSelected ? 'text-white/80' : 'text-slate-500'}`} /></button>
                                                <button disabled title="Desativado no modo de demonstração" className="p-1.5 rounded-md"><TrashIcon className={`w-4 h-4 ${isSelected ? 'text-rose-300' : 'text-rose-500'}`} /></button>
                                            </div>
                                        </li>
                                    );
                                })}
                ...
                <main className="w-full md:w-2/3 lg:w-3/4 py-2">
                     <div className="mb-4 p-4 text-center bg-amber-100/80 dark:bg-amber-500/10 text-amber-800 dark:text-amber-300 rounded-lg border border-amber-200 dark:border-amber-500/20 flex items-center justify-center gap-3">
                        <LightbulbIcon className="w-5 h-5 flex-shrink-0"/>
                        <span className="text-sm font-medium">Modo Demonstração: Funcionalidades de modificação e geração de relatórios estão desativadas.</span>
                    </div>
                    {selectedSaleswoman ? (
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <div className="flex items-center gap-2 mt-1">
                                                {selectedSaleswoman.summaryLastGeneratedAt && <Badge color="slate">Último resumo: {new Date(selectedSaleswoman.summaryLastGeneratedAt).toLocaleDateString('pt-BR')}</Badge>}
                                                {canDownload && <Badge color="green">PDF disponível</Badge>}
                                            </div>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <button disabled title="Desativado no modo de demonstração" className="inline-flex items-center gap-2 text-sm font-medium px-4 py-2 rounded-lg bg-primary-600 text-white disabled:bg-slate-400 dark:disabled:bg-slate-600 disabled:cursor-not-allowed"><DocumentPlusIcon className="w-5 h-5" /> Gerar Resumo</button>
                                        <button disabled title="Desativado no modo de demonstração" className="inline-flex items-center gap-2 text-sm font-medium px-4 py-2 rounded-lg bg-slate-100 dark:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed"><ArrowDownTrayIcon className="w-5 h-5" /> Baixar PDF</button>
                                        <button disabled title="Desativado no modo de demonstração" className="inline-flex items-center gap-2 text-sm font-medium px-4 py-2 rounded-lg bg-slate-100 dark:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed"><PaperAirplaneIcon className="w-5 h-5" /> Enviar Email</button>
                                    </div>
                                </div>
